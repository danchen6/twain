...
- Analytics tests enforce disabled/config/consent initialization gates, versioned timestamped consent validation, one lazy Google tag, privacy-limited configuration/revocation, bounded GA-compatible scalar events, and the absence of paths, seeds, or identifiers from Twain event builders.
...
- initial privacy banner layout at 1440×1000, 768×1024, 390×844, and 320×800; localized details modal and Help re-entry; direct accept/decline persistence; hidden banner after reload; no Google tag/data layer while configuration is disabled; and streak creation only after restored or newly solved full-daily completion;
...
| Privacy/analytics | undecided/declined/granted persistence, no pre-consent or declined tag request, details and withdrawal path, clean reload after active revocation, GA DebugView validation before production enablement |
