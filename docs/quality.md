...
- Independent clue order, clue ownership, shared-cell rejection, early line completion, immediate-predecessor backtracking, immutable direct path-cell rewind, older-body rejection, global Undo, and Hint correction.
...
- Analytics tests lock the approved production stream and debug-off setting, enforce configuration/consent initialization gates, v2 timestamp validation, persistent grants, session-only denials, v1 invalidation, one lazy Google tag, Google's non-Array `Arguments` command shape, privacy-limited configuration/revocation, bounded GA-compatible scalar events, and the absence of paths, seeds, or identifiers from Twain event builders.
...
- initial privacy banner and Enhanced Measurement details layout at 1440×1000, 768×1024, 390×844, and 320×800, including the 320px Brazilian Portuguese details copy; Help re-entry; persistent grants and session-only denials; same-session declined reload plus a simulated next-session re-prompt; no pre-consent or declined Google tag/data layer/request; configured-tag initialization only after a grant; clean tag-free reload after active revocation; and streak creation only after restored or newly solved full-daily completion;
...
| Surface | Checks |
| --- | --- |
| Desktop pointer | daily toolbar order, orthogonal click drawing, direct path-click rewind, cell-by-cell tail drag-back, non-predecessor drag-body no-op, line selection/fading, timer-preserving Clear, Undo, Hint, on-board Next level, Help dialog and Privacy choices, initial privacy bar, Language menu, header QR/link dialog and Copy, result system Share/fallback |
