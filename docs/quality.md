...
- Finished-result copy formats zero, singular, and plural Hint counts; invalid counters are rejected.
...
- Static deployment remains relative, daily-only, free of query-controlled puzzle selection, and free of package-manager/CDN/runtime-network dependencies; the pinned QR source and license must remain present.
...
| Surface | Checks |
| --- | --- |
| Desktop pointer | daily toolbar order, click/drag, line selection/fading, backtracking/body rejection, timer-preserving Clear, Undo, Hint, on-board Next level, Help dialog, header QR/link dialog and Copy, result system Share/fallback |
| Mobile touch | crisp two-stroke logo and full header wordmark, visible timer icon, black Help control, Help and header Share dialog containment, QR scanning and URL long-press selection, no page scroll while drawing, double-tap/long-press policy outside copy fields, cells reachable, toolbar/progress unclipped, Ultra touch and Clear |
...
The harness proves that header Share does not call `navigator.share()`, that its rendered QR and visible link derive from the same canonical URL, and that a real result-Share click reaches `navigator.share()` while transient activation is active. It cannot independently exercise a physical camera scanner or inspect the operating-system result share sheet; those final device-owned surfaces still require physical checks.
...
- A local HTTP server returns `index.html`, `styles.css`, every imported module, and the pinned QR encoder.
