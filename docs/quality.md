...
- Analytics tests lock the approved production stream and debug-off setting, enforce configuration/consent initialization gates, v2 timestamp validation, persistent grants, session-only denials, v1 invalidation, one lazy Google tag, privacy-limited configuration/revocation, bounded GA-compatible scalar events, and the absence of paths, seeds, or identifiers from Twain event builders.
