...
- `src/analytics.js`: disabled-by-default GA configuration, versioned consent-record parsing/writing, scalar event sanitization, and lazy Google tag transport. It creates no tag state or request until configuration and explicit permission both pass.
...
`twain:analytics-consent:v1` contains a schema version, `granted`/`denied` choice, and ISO decision time. Missing, malformed, or unwritable consent is fail-closed. It is a local preference record rather than a remote audit log. Functional play, locale, and streak storage are independent of this optional analytics choice.
...
The analytics boundary is outcome-only. `main.js` strips query/fragment state before initialization, `telemetry.js` never receives paths, moves, seeds, clue values, text input, or identity, and `analytics.js` admits only bounded scalar parameters. A consented GA tag can still collect Google's standard page/device/browser and approximate-geography context. Ad storage, ad user data, ad personalization, Google signals, and ad-personalization signals remain disabled. See [analytics.md](analytics.md) for the exact schema and limitations.
