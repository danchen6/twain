...
- One centered release identifier follows the game card in a semantic footer. It uses the subtle neutral token, 11px tabular sans-serif type, modest tracking, and no surrounding label, link, border, or panel. It is source metadata rather than localized copy and must remain visually subordinate to the board and controls.
...
2. One centered game card.
   ...
   - Undo and Hint.
3. One quiet app-revision footer below the game card.
4. Anchored Language menu, the initial bottom privacy bar, and modal localized **How to play**, Privacy details, header QR/link Share, and finished-result sharing-fallback dialogs outside the play layout.

There is no mode picker, difficulty picker, New/Replay action, visible line selector, occupancy panel, visible status area, visible progress copy, stage/grid/wall metadata, hero copy, or inline rules panel. A visually hidden live announcer retains rule and action feedback without occupying layout. The puzzle must remain playable without opening the tutorial.
...
- The in-flow release footer stays below the game card, never overlays play or the fixed privacy bar, and may fall below the initial fold rather than shrinking the board. While the initial privacy bar would cover its in-flow position at widths above 620px, the footer stays hidden until the player makes a choice; narrower layouts retain enough clearance to show both.
...
A rendered state is not acceptable merely because it builds. Visual QA judges hierarchy, route/grid/wall geometry, spacing rhythm, clue legibility/alignment, dynamic daily progress clarity, small-size logo legibility, header wordmark/date/action separation, release-footer restraint and containment, Language-menu containment and selection, localized tutorial/privacy/QR-link dialog containment, privacy-bar readability/action parity/underlay reachability, QR contrast/quiet zone, selectable URL fit, narrow-width overflow, focus/disabled/error/hint/completion states, overlay containment and celebration motion, content coherence, and regressions against this contract and the prior clean capture in the same loop.
